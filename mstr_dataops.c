...
void recursive_merge(json_t *dest, json_t *src)
{
	...
	/* Will not mow over different data types. This function is only useful
	 * if the target and destination have the same structure. */
	...
	case JSON_OBJECT:
		...
		break;
	case JSON_ARRAY:
		for(index=0; index < json_array_size(src); index += 1) {
			value = json_array_get(src, index);
