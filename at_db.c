...
int snap_rec_tbl(obj_rec_t *obj_rec)
{
	...
	size_t i;
	char *nullobj = strdup("{}");
	char *sysload, *memory, *cpu_ttls, *cpu_static, *cpus, *iface, *iodev, *fsinf, *meta, *misc;
	const time_t nownow = time(NULL);
	json_t *jts;
	struct timeval ts;
	ExecStatusType pqrc;
	...
	if(cpu_static == NULL) cpu_static = nullobj;
	...
	if(memory != nullobj) free(memory);
	if(cpu_ttls != nullobj) free(cpu_ttls);
	if(cpu_static != nullobj) free(cpu_static);
	if(cpus != nullobj) free(cpus);
	if(iface != nullobj) free(iface);
	if(iodev != nullobj) free(iodev);
...
void *database_ops_queue(void *dptr)
{
	...
	while(on) {
		...
		if(processed > 0) syslog(LOG_DEBUG, "%li record(s) committed this queue cycle", processed);
